...
export default function Profile() {
  ...
  const loadProfile = async () => {
    try {
      ...
      const record = await pb.collection('users').getOne(current.id);
  ...
  const handleAvatarUpload = async (event) => {
    ...
    try {
      ...
      const updated = await pb.collection('users').update(current.id, form);
      pb.authStore.save(pb.authStore.token, updated);

      const normalized = normalizeUser(updated);
      setProfile((prev) => ({ ...prev, avatar: updated.avatar, avatar_url: normalized.avatar_url }));
              ...
              onBlur={async (e) => {
                ...
                try {
                  const found = await pb
                    .collection('users')
                    .getFirstListItem(
                      `username = "${e.target.value}" && id != "${user.id}"`
                    );
                  if (found) {
